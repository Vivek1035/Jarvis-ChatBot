...
def wishMe():
    ...
    if(hour>=0) and (hour<=12) and ('AM' in t):
        speak(f"Good Morning Vivek, it's {day} and the time is {t}")
    elif(hour>=12)  and (hour<=16) and ('PM' in t):
        speak(f"Good Afternoon Vivek, it's {day} and the time is {t}")
    else:
        speak(f"Good Evening Vivek, it's {day} and the time is {t}")
